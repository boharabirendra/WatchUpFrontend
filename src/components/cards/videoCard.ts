...
export const VideoCard = (video: IVideo) => `
  <div id="video-item" class="group cursor-pointer flex flex-col w-full max-w-xs transition-all duration-300 hover:transform hover:scale-105" 
    ...
    >
    <div class="relative w-full h-56 overflow-hidden rounded-lg shadow-lg bg-gray-800">
    ...
    <div class="mt-4 flex items-start">
      <div class="flex-shrink-0"> 
        <img src="${video.userVideos[0].profileUrl || '/public/sample.jpeg'}" class="h-10 w-10 rounded-full object-cover" alt="${video.userVideos[0].email}" />
      </div>
